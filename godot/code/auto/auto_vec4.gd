## Automates a [Vector4] value.
class_name AutoVector4
extends Auto
...
var current_state: Vector4State:
	get:
		return mode.automate_vec4(initial, desired, elapsed)
...
func reset_to(target: Vector4) -> void:
	initial = current_state
	desired = target
	elapsed = 0
...
## accounted for.
func update(delta: float) -> Vector4:
	elapsed += delta
	return current
...
func _init(initial_value: Vector4, auto_mode: Mode) -> void:
	mode = auto_mode
	initial = Vector4State.new(initial_value, Vector4.ZERO)
	desired = initial_value

func _to_string() -> String:
	return "%s, Vector4 %s -> %s: %s" % \
